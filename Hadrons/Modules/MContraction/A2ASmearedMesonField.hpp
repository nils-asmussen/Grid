...
void TA2ASmearedMesonField<FImpl>::setup(void)
{
    ...
    envTmp(std::vector<ComplexField>, "distrib", 1, distributionsMap_.size(),
...
void TA2ASmearedMesonField<FImpl>::execute(void)
{
    ...
    envGetTmp(std::vector<ComplexField>, distrib);
    envGetTmp(FFT, fft);
...
//out[n*Nm+m](q)=distribLeft[n](q)*distribRight[n](q+p-smearMom[m])*invSpVol
//where
//Nm=smearMom.size()
...
{
    using std::get;
    assert(out.size() == distrib.size()*smearMom.size());
